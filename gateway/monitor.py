"""
Мониторинг системы: GPU через nvidia-smi, CPU/RAM через psutil, статус Ollama, очереди задач. Следит за воркерами и перезапускает при падении.

format_status -- главная функция, вызывается таймером каждые 5 сек, возвращает строку для UI.
"""
