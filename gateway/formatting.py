...
def render_topics(topics):
    """Рендерит список тем как цветные чипы (HTML)."""
    if not topics:
        return ""
    chips = []
...
def render_word_stats(segments):
    """Рендерит строку статистики: слова, длительность, время чтения (HTML)."""
    words = sum(
...
def render_kb_stats(count, total_words, total_audio_sec):
    """Рендерит шапку базы знаний со статистикой (HTML)."""
    if count == 0:
...
def render_asr_progress():
    """Прогресс-бар транскрипции (indeterminate — пульсирует)."""
    return (
