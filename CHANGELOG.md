...
| До | После |
| --- | --- |
| ![До интерфейса](before.png) | ![После интерфейса](after.png) |
...
Я бы отправлял именно как файл `CHANGELOG.md` в репозитории. **Release не создавай**, чтобы не выглядеть так, будто ты оформляешь версию готового продукта. Для практики это должен быть просто журнал изменений проекта.
```
